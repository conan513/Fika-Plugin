#!/usr/bin/env bash
set -euo pipefail

# ANSI color codes
CYAN='\033[0;36m'
YELLOW='\033[1;33m'
GREEN='\033[0;32m'
RED='\033[0;31m'
GRAY='\033[0;90m'
NC='\033[0m' # No Color

PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CSPROJ_FILE="${PROJECT_DIR}/Fika.Core/Fika.Core.csproj"
DIST_DIR="${PROJECT_DIR}/dist"
DEFAULT_SPT_PATH="${SPT_INSTALL_PATH:-/home/conan/Games/SPP-EFT}"

CONFIGURATION="Release"
CLEAN=false
NO_DEPLOY=false
SPT_PATH="${DEFAULT_SPT_PATH}"

usage() {
    cat <<EOF
Fika-Plugin Build & Package Script (Linux)

Usage:
  ./build.sh [options]

Options:
  -c, --configuration <Config>  Build configuration: 'Release' (default) or 'Debug'
  --clean                       Performs a clean build (removes bin and obj folders)
  --no-deploy                   Skips copying to the SPT game plugins directory
  --spt-path <Path>             Target SPT installation path (default: '${DEFAULT_SPT_PATH}')
  -h, --help                    Show this help message
EOF
}

while [[ $# -gt 0 ]]; do
    case "$1" in
        -c|--configuration)
            if [[ "$2" =~ ^(Release|Debug|GoldMaster)$ ]]; then
                CONFIGURATION="$2"
                shift 2
            else
                echo -e "${RED}[ERROR] Invalid configuration '$2'. Allowed values: Release, Debug, GoldMaster.${NC}" >&2
                exit 1
            fi
            ;;
        --clean)
            CLEAN=true
            shift
            ;;
        --no-deploy)
            NO_DEPLOY=true
            shift
            ;;
        --spt-path)
            SPT_PATH="$2"
            shift 2
            ;;
        -h|--help)
            usage
            exit 0
            ;;
        *)
            echo -e "${RED}[ERROR] Unknown option: $1${NC}" >&2
            usage
            exit 1
            ;;
    esac
done

echo -e "${CYAN}==========================================${NC}"
echo -e "${CYAN}        Fika-Plugin Build Script          ${NC}"
echo -e "${CYAN}==========================================${NC}"
echo -e "${GRAY}Configuration : ${CONFIGURATION}${NC}"
echo -e "${GRAY}Project       : ${CSPROJ_FILE}${NC}"

# 1. Clean
if [ "$CLEAN" = true ]; then
    echo -e "\n${YELLOW}[1/4] Cleaning project artifacts...${NC}"
    dotnet clean "${CSPROJ_FILE}" -c "${CONFIGURATION}" --verbosity quiet
    rm -rf "${PROJECT_DIR}/Fika.Core/bin" "${PROJECT_DIR}/Fika.Core/obj" "${DIST_DIR}/staging"
    echo -e "${GREEN}Clean completed.${NC}"
else
    echo -e "\n${GRAY}[1/4] Skipping clean (use --clean to force).${NC}"
fi

# 2. Build
echo -e "\n${YELLOW}[2/4] Building Fika.Core (${CONFIGURATION})...${NC}"
BUILD_FLAGS=("-c" "${CONFIGURATION}")
if [ "$CLEAN" = true ]; then
    BUILD_FLAGS+=("--no-incremental")
fi

dotnet build "${CSPROJ_FILE}" "${BUILD_FLAGS[@]}"

OUT_DIR="${PROJECT_DIR}/Fika.Core/bin/${CONFIGURATION}/netstandard2.1"
DLL_PATH="${OUT_DIR}/Fika.Core.dll"
if [ ! -f "${DLL_PATH}" ]; then
    echo -e "${RED}[ERROR] Compiled DLL not found at: ${DLL_PATH}${NC}" >&2
    exit 1
fi

VERSION="2.4.3"
echo -e "${GRAY}Compiled binary: Fika.Core.dll (v${VERSION})${NC}"

# 3. Direct Deploy
if [ "$NO_DEPLOY" = false ]; then
    echo -e "\n${YELLOW}[3/4] Deploying to SPT game folder...${NC}"
    PLUGIN_TARGET_DIR="${SPT_PATH}/BepInEx/plugins/Fika"

    if [ -d "${SPT_PATH}/BepInEx" ] || [ -d "${SPT_PATH}/BepInEx/plugins" ]; then
        mkdir -p "${PLUGIN_TARGET_DIR}"
        cp -f "${DLL_PATH}" "${PLUGIN_TARGET_DIR}/Fika.Core.dll"
        [ -f "${OUT_DIR}/Fika.Core.pdb" ] && cp -f "${OUT_DIR}/Fika.Core.pdb" "${PLUGIN_TARGET_DIR}/"
        for lic in "${PROJECT_DIR}"/LICENSE*.md "${PROJECT_DIR}"/Licenses/*.md; do
            [ -f "$lic" ] && cp -f "$lic" "${PLUGIN_TARGET_DIR}/" 2>/dev/null || true
        done
        echo -e "${GREEN}Successfully deployed to: ${PLUGIN_TARGET_DIR}${NC}"
    else
        echo -e "${YELLOW}[WARN] SPT install path not found at '${SPT_PATH}'. Skipped copy.${NC}"
    fi
else
    echo -e "\n${GRAY}[3/4] Skipping deployment (--no-deploy specified).${NC}"
fi

# 4. Packaging
echo -e "\n${YELLOW}[4/4] Creating distribution package in /dist...${NC}"
mkdir -p "${DIST_DIR}"

STAGING_ROOT="${DIST_DIR}/staging"
PACKAGE_STAGING="${STAGING_ROOT}/BepInEx/plugins/Fika"

rm -rf "${STAGING_ROOT}"
mkdir -p "${PACKAGE_STAGING}"

cp -f "${DLL_PATH}" "${PACKAGE_STAGING}/Fika.Core.dll"
[ -f "${OUT_DIR}/Fika.Core.pdb" ] && cp -f "${OUT_DIR}/Fika.Core.pdb" "${PACKAGE_STAGING}/"
for lic in "${PROJECT_DIR}"/LICENSE*.md "${PROJECT_DIR}"/Licenses/*.md; do
    [ -f "$lic" ] && cp -f "$lic" "${PACKAGE_STAGING}/" 2>/dev/null || true
done

ZIP_FILE_NAME="Fika-Plugin-v${VERSION}.zip"
ZIP_FILE_PATH="${DIST_DIR}/${ZIP_FILE_NAME}"

rm -f "${ZIP_FILE_PATH}"
if command -v zip >/dev/null 2>&1; then
    (cd "${STAGING_ROOT}" && zip -r -q "${ZIP_FILE_PATH}" .)
else
    python3 -c "import shutil; shutil.make_archive('${ZIP_FILE_PATH%.zip}', 'zip', '${STAGING_ROOT}')"
fi
rm -rf "${STAGING_ROOT}"

echo -e "${GREEN}Package created: dist/${ZIP_FILE_NAME}${NC}"
echo -e "\n${CYAN}==========================================${NC}"
echo -e "${CYAN}              BUILD COMPLETE!             ${NC}"
echo -e "${CYAN}==========================================${NC}"
